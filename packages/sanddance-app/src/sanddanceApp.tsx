...
import { useAuth } from "universal-keycloak-auth/react";
...
export interface Props {
  ...
  mounted?: (app: SandDanceAppInner) => void;
  insights?: InsightMap;
...
if (
  ...
  snapshotOnLoad.dataSource.dataSourceType === "local"
) {
  snapshotOnLoad = null;
}
...
export class SandDanceAppInner extends React.Component<Props, State> {
  ...
  constructor(props: Props) {
    ...
    this.state = {
      ...
      dialogMode: null,
      dataSource:
        (snapshotOnLoad && snapshotOnLoad.dataSource) || props.dataSources[0],
  ...
  changeColorScheme(darkTheme: boolean) {
    ...
    SandDance.VegaMorphCharts.base.vega.scheme(
        ...
        <Explorer
          ...
          dataExportHandler={(data, datatype, displayName) => {
            try {
              ...
              this.explorer.setState({
                errors: [strings.errorDownloadFailure],
              });
            }
          }}
          datasetElement={
...
export const SandDanceApp = (props: Props) => {
  ...
  if (!isAuthenticated) {
    return (
      ...
      >
        ...
        <h2 style={{ margin: "0 0 8px 0", color: "#323130" }}>
          Access Restricted
        </h2>
        <p style={{ margin: "0 0 24px 0", color: "#605e5c" }}>
          Please log in to continue to SandDance Nova.
        </p>
  ...
  return <SandDanceAppInner {...props} />;
};
